...
def estado_hidratacion(consumo_ml, objetivo_ml):
    ...
    if consumo_ml < objetivo_ml:
        ...
        return f"Excediste tu objetivo en {porcentaje:.1f}%"

personas = []

while True:
    try:
        peso = float(input("Ingrsá tu peso en kg: "))
        if peso <= 0:
            raise ValueError("El peso tiene que ser un número positivo")

        actividad = input("Ingresá tu nivel de actividad física (bajo, medio, alto): ").lower()
        if actividad not in ["bajo", "medio", "alto"]:
            raise ValueError("Nivel de actividad tiene que ser 'bajo', 'medio' o 'alto'")

        consumo = float(input("Ingresá la cantidad de agua que ha consumido hoy en ml: "))
        if consumo < 0:
        ...
        continuar = input("¿Queres agregar otra persona? (s/n): ").lower()
        if continuar != 's':
            break
